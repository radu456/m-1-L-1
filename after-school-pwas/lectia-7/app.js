...
(() => {
  ...
  const defaultState = {
    current: 0, recap: recapDefault, checklist: {}, checks: {},
  ...
  function checklist() {
    ...
    const verified=!!state.checks.checklist;
    return `<section class="content-card"><p class="step-kicker">3 · Testul „Am tot ce îmi trebuie?”</p><h2>Verifică planul de autor</h2>${introBlock('Bifează ce ai deja, apoi apasă „Verifică planul”. Dacă lipsește ceva, nicio problemă: te poți întoarce la recapitulare.')}<div class="check-grid">${checklistItems.map(([key,label]) => check(`checklist.${key}`, label)).join('')}</div><button class="btn primary" data-action="check-checklist">Verifică planul</button>${verified?`<div class="feedback ${all ? 'good' : 'warn'}">${all ? '🎉 Perfect! Ești pregătit să scrii!' : `${done}/${checklistItems.length} bifate. Completează elementele lipsă înainte de a începe.`}</div>`:''}${verified&&!all ? '<button class="btn ghost" data-action="jump-missing" style="margin-top:12px">Mergi la recapitulare</button>' : ''}</section>`;
  }

  function titleRead() { const verified=!!state.checks.titleRead, picked=getPath('exampleTitle'), right='usa'; return `<section class="content-card"><p class="step-kicker">4 · Găsește titlul</p><h2>Titlul este prima impresie</h2>${introBlock('Un titlu bun îl face pe cititor să vrea să afle mai mult. Privește exemplul și alege titlul care se potrivește, apoi verifică.')}<div class="story"><p><strong>Poveste:</strong> „Un copil găsește o ușă secretă într-o pădure și descoperă o lume magică.”</p></div><fieldset class="question"><legend>Care este un titlu potrivit?</legend><div class="choice-list">${[['copil','Copilul'],['padure','Pădurea'],['usa','Ușa din Pădurea Fermecată'],['zi','O zi obișnuită']].map(([v,l])=>`<label class="choice ${verified&&(picked===v||v===right)?(v===right?'is-correct':'is-wrong'):''} ${verified&&v===right&&picked!==right?'correct-answer':''}"><input type="radio" data-choice="exampleTitle" name="exampleTitle" value="${v}" ${picked===v?'checked':''}><span>${l}</span>${verified&&v===right?'<span class="answer-mark">✓</span>':verified&&picked===v?'<span class="answer-mark">✕</span>':''}</label>`).join('')}</div><button class="btn primary" data-action="check-title">Verifică titlul</button>${verified?`<div class="answer-explanation ${picked===right?'good':'bad'}">${picked===right?'✅ Corect! Titlul arată ușa secretă și locul magic.':'❌ Răspunsul corect este: <strong>Ușa din Pădurea Fermecată</strong>.'}</div>`:''}</fieldset></section>`; }
  ...
  function curious() { const verified=!!state.checks.curious, title=getPath('curious'), why=getPath('curiousWhy'), titleOk=title==='dincolo', whyOk=why==='dorinta'; return `<section class="content-card"><p class="step-kicker">6 · Curiozitatea cititorului</p><h2>Care titlu te face curios?</h2>${introBlock('Povestea este despre un copil care găsește un portal către o altă lume. Alege titlul și motivul, apoi apasă „Verifică”.')}<fieldset class="question"><legend>Alege titlul</legend><div class="choice-list">${[['poveste','„O poveste”'],['portal','„Portalul”'],['dincolo','„Ce se afla dincolo de portal?”'],['copil','„Copilul”']].map(([v,l])=>`<label class="choice ${verified&&(title===v||v==='dincolo')?(v==='dincolo'?'is-correct':'is-wrong'):''} ${verified&&v==='dincolo'&&!titleOk?'correct-answer':''}"><input type="radio" data-choice="curious" name="curious" value="${v}" ${title===v?'checked':''}><span>${l}</span>${verified&&v==='dincolo'?'<span class="answer-mark">✓</span>':verified&&title===v?'<span class="answer-mark">✕</span>':''}</label>`).join('')}</div></fieldset><fieldset class="question" style="margin-top:14px"><legend>De ce?</legend><div class="choice-list">${[['dorinta','Pentru că ne face să vrem să aflăm ce se întâmplă.'],['scurt','Pentru că este cel mai scurt.'],['cuvinte','Pentru că are mai multe cuvinte.']].map(([v,l])=>`<label class="choice ${verified&&(why===v||v==='dorinta')?(v==='dorinta'?'is-correct':'is-wrong'):''} ${verified&&v==='dorinta'&&!whyOk?'correct-answer':''}"><input type="radio" data-choice="curiousWhy" name="curiousWhy" value="${v}" ${why===v?'checked':''}><span>${l}</span>${verified&&v==='dorinta'?'<span class="answer-mark">✓</span>':verified&&why===v?'<span class="answer-mark">✕</span>':''}</label>`).join('')}</div></fieldset><button class="btn primary" data-action="check-curious">Verifică alegerile</button>${verified?`<div class="answer-explanation ${titleOk&&whyOk?'good':'bad'}">${titleOk&&whyOk?'✅ Exact! Titlul deschide o întrebare în mintea cititorului.':'❌ Răspunsurile corecte sunt „Ce se afla dincolo de portal?” și „Pentru că ne face să vrem să aflăm ce se întâmplă.”'}</div>`:''}</section>`; }
  ...
  function verify() { const done = verifyItems.filter(([key]) => checked(`verify.${key}`)).length; const all = done === verifyItems.length; const verified=!!state.checks.verify; return `<section class="content-card"><p class="step-kicker">16 · Verifică povestea ca un autor adevărat</p><h2>🔎 Ultima revizie</h2>${introBlock('Bifează fiecare lucru pe care l-ai verificat, apoi apasă butonul. Nu căutăm perfecțiune, ci o poveste care se înțelege și îți aparține.')}<div class="check-grid">${verifyItems.map(([key,label]) => check(`verify.${key}`, label)).join('')}</div><button class="btn primary" data-action="check-verify">Verifică povestea</button>${verified?`<div class="feedback ${all ? 'good' : 'warn'}">${all ? '✅ Povestea ta este pregătită pentru copertă și prezentare!' : `${done}/${verifyItems.length} verificate. Mai ai puțin.`}</div>`:''}</section>`; }
  ...
  function bind() {
    ...
    app.querySelectorAll('[data-choice]').forEach(el => el.addEventListener('change', event => { setPath(event.target.dataset.choice, event.target.value); if (['exampleTitle','curious','curiousWhy'].includes(event.target.dataset.choice)) state.checks[event.target.dataset.choice === 'exampleTitle' ? 'titleRead' : 'curious'] = false; save(); render(); }));
    app.querySelectorAll('[data-check]').forEach(el => el.addEventListener('change', event => { setPath(event.target.dataset.check, event.target.checked); if (event.target.dataset.check.startsWith('checklist.')) state.checks.checklist=false; if (event.target.dataset.check.startsWith('verify.')) state.checks.verify=false; save(); render(); }));
    app.querySelectorAll('[data-action]').forEach(el => el.addEventListener('click', event => {
      ...
      if (action === 'check-title') { state.checks.titleRead=true; save(); render(); }
      if (action === 'check-curious') { state.checks.curious=true; save(); render(); }
      if (action === 'check-checklist') { state.checks.checklist=true; save(); render(); }
      if (action === 'check-verify') { state.checks.verify=true; save(); render(); }
