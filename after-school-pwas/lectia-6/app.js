...
(() => {
  ...
  function intelegere() {
    ...
    const verified = !!state.verified.intelegere;
    return `<section class="content-card"><p class="step-kicker">2 · Înțelegerea poveștii</p><h2>Verificăm aventura lui Max</h2><p class="intro">Alege un răspuns la fiecare întrebare, apoi apasă „Verifică”.</p><div class="question-grid">${qData.map(([id, text, opts, good], i) => `<fieldset class="question"><legend>Întrebarea ${i + 1}. ${esc(text)}</legend><div class="choice-list">${opts.map(([v,l]) => { const picked=answer(id,v), isRight=v===good; const cls=verified&&(picked||isRight)?(isRight?'is-correct':'is-wrong'):''; return `<label class="choice ${cls} ${verified&&isRight&&!picked?'correct-answer':''}"><input type="radio" name="${esc(id)}" data-choice="${esc(id)}" value="${esc(v)}" ${picked?'checked':''}><span>${esc(l)}</span>${verified&&isRight?'<span class="answer-mark">✓</span>':verified&&picked?'<span class="answer-mark">✕</span>':''}</label>`; }).join('')}</div>${verified?`<div class="answer-explanation ${answer(id,good)?'good':'bad'}">${answer(id,good)?'✅ Corect!':`❌ Răspunsul corect este: <strong>${esc(opts.find(o=>o[0]===good)?.[1]||'')}</strong>.`}</div>`:''}</fieldset>`).join('')}</div><div class="nav-group" style="margin-top:14px"><button class="btn primary" data-action="check-quiz6">Verifică răspunsurile</button></div>${verified?`<div class="feedback ${answered===qData.length&&correct===qData.length?'good':'bad'}">Ai răspuns la ${answered}/${qData.length}. Corecte: ${correct}/${qData.length}. ${correct===qData.length?'🎉':'Răspunsurile corecte sunt verzi, iar cele greșite au explicația dedesubt.'}</div>`:''}</section>`;
  }

  function piese() {
    const verified=!!state.verified.piesa;
    const pieces=[['inceput','🟢 Începutul'],['problema','🟡 Problema'],['moment','🟠 Momentul important'],['final','🔵 Finalul']];
    const options=pieces.map(([value,label])=>{ const right=value==='moment'; const picked=answer('piesa',value); const cls=verified&&(picked||right)?(right?'is-correct':'is-wrong'):''; return `<label class="choice ${cls} ${verified&&right&&!picked?'correct-answer':''}"><input type="radio" name="piesa" data-choice="piesa" value="${value}" ${picked?'checked':''}><span>${label}</span>${verified&&right?'<span class="answer-mark">✓</span>':verified&&picked?'<span class="answer-mark">✕</span>':''}</label>`;}).join('');
    const result=verified?`<div class="answer-explanation ${answer('piesa','moment')?'good':'bad'}">${answer('piesa','moment')?'✅ Corect! Momentul important arată unde personajul acționează sau ia o decizie.':'❌ Răspunsul corect este: <strong>Momentul important</strong>.'}</div>`:'';
    return `<section class="content-card"><p class="step-kicker">3 · Puzzle-ul poveștii</p><h2>Cele 4 piese ale unei povești</h2><p class="intro">O poveste poate fi construită ca un puzzle. Fiecare piesă ne ajută să urmărim aventura.</p><div class="puzzle-grid"><article class="puzzle"><strong>🟢 1. ÎNCEPUTUL</strong><small>Îl cunoaștem pe personaj și aflăm unde este.</small></article><article class="puzzle"><strong>🟡 2. PROBLEMA</strong><small>Se întâmplă ceva și personajul are o misiune.</small></article><article class="puzzle"><strong>🟠 3. MOMENTUL IMPORTANT</strong><small>Apare obstacolul, iar personajul ia o decizie.</small></article><article class="puzzle"><strong>🔵 4. FINALUL</strong><small>Aflăm cum se termină aventura.</small></article></div><div class="tip">În povestea lui Max: tunelul este începutul, podul rupt este problema, frânghia este momentul important, iar pădurea este finalul.</div><h3>Mini-provocare</h3><p>Care piesă arată soluția sau decizia personajului?</p><div class="choice-list">${options}</div><button class="btn primary" data-action="check-piesa">Verifică piesa</button>${result}</section>`;
  }
  ...
  function ordine() {
    const checked=!!state.orderChecked, goodOrder=state.order.join('')==='BCAD';
    return `<section class="content-card"><p class="step-kicker">4 · Ordonează povestea</p><h2>Pune evenimentele în ordinea corectă</h2><p class="intro">Trage cardurile sau folosește săgețile pentru a le muta. Pe tabletă, butoanele sunt mai ușor de folosit.</p><div class="order-zone ${checked?(goodOrder?'order-correct':'order-wrong'):''}" data-order-zone>${state.order.map((letter, index) => `<div class="order-card" draggable="true" data-letter="${letter}" data-index="${index}"><span class="order-letter">${letter}</span><span>${esc(orderText[letter])}</span><button class="move-btn" data-move="up" data-index="${index}" aria-label="Mută în sus" ${index === 0 ? 'disabled' : ''}>↑</button><button class="move-btn" data-move="down" data-index="${index}" aria-label="Mută în jos" ${index === state.order.length - 1 ? 'disabled' : ''}>↓</button></div>`).join('')}</div><div class="nav-group" style="margin-top:14px"><button class="btn primary" data-action="check-order">Verifică ordinea</button></div>${checked ? `<div class="answer-explanation ${goodOrder?'good':'bad'}">${goodOrder?'✅ Bravo! Ordinea este B → C → A → D.':'❌ Ordinea corectă este: <strong>B → C → A → D</strong> — mai întâi intră în tunel, apoi vede podul rupt, găsește frânghia și ajunge în pădure.'}</div>` : '<p class="status">După verificare vei vedea ordinea corectă.</p>'}</section>`;
  }
  ...
  function sortare() {
    ...
    const verified=!!state.sortChecked;
    return `<section class="content-card"><p class="step-kicker">5 · Sortează</p><h2>Început, mijloc sau final?</h2><p class="intro">Alege categoria pentru fiecare propoziție, apoi apasă „Verifică sortarea”.</p><div class="sort-grid">${sortItems.map(([id,text,answerKey]) => {const right=state.sort[id]===answerKey;return `<div class="sort-item ${verified?(right?'is-correct':'is-wrong'):''}"><p><strong>${id}.</strong> ${esc(text)}</p><select data-sort="${id}" aria-label="Categoria propoziției ${id}"><option value="">Alege categoria…</option><option value="inceput" ${state.sort[id] === 'inceput' ? 'selected' : ''}>Început</option><option value="mijloc" ${state.sort[id] === 'mijloc' ? 'selected' : ''}>Mijloc</option><option value="moment" ${state.sort[id] === 'moment' ? 'selected' : ''}>Moment important</option><option value="final" ${state.sort[id] === 'final' ? 'selected' : ''}>Final</option></select>${verified&&!right?`<div class="answer-explanation bad">❌ Corect: <strong>${answerKey==='inceput'?'Început':answerKey==='mijloc'?'Mijloc':answerKey==='moment'?'Moment important':'Final'}</strong>.</div>`:verified?'<div class="answer-explanation good">✅ Corect!</div>':''}</div>`;}).join('')}</div><button class="btn primary" data-action="check-sort">Verifică sortarea</button>${verified?`<div class="feedback ${good===sortItems.length?'good':'bad'}">${Object.keys(state.sort).length}/${sortItems.length} completate · ${good}/${sortItems.length} corecte.</div>`:''}</section>`;
  }
  ...
  function lipseste() {
    ...
    const verified=!!state.verified.missing;
    const opts=[['obstacol','O problemă sau un obstacol'],['moment','Un moment important'],['detalii','Mai multe detalii despre aventură'],['personaje','Un număr foarte mare de personaje']];
    return `<section class="content-card"><p class="step-kicker">6 · Gândește ca un autor</p><h2>Ce lipsește?</h2><div class="story"><p>„Andrei găsește o hartă misterioasă. Pleacă să caute comoara. După un timp, se întoarce acasă.”</p></div><p class="intro">Bifează toate ideile care ar face povestea mai interesantă, apoi verifică.</p><div class="choice-list">${opts.map(([v,l])=>{const picked=selected('missing',v),right=correct.includes(v),cls=verified&&(picked||right)?(right?'is-correct':'is-wrong'):'';return `<label class="choice ${cls} ${verified&&right&&!picked?'correct-answer':''}"><input type="checkbox" data-multi="missing" value="${v}" ${picked?'checked':''}><span>${l}</span>${verified&&right?'<span class="answer-mark">✓</span>':verified&&picked?'<span class="answer-mark">✕</span>':''}</label>`;}).join('')}</div><button class="btn primary" data-action="check-missing">Verifică ideile</button>${verified?`<div class="answer-explanation ${ok?'good':'bad'}">${ok?'✅ Exact! Povestea are nevoie de obstacol, moment important și detalii.':'❌ Răspunsurile corecte sunt: <strong>obstacol, moment important și detalii</strong>. „Multe personaje” nu este obligatoriu.'}</div>`:''}</section>`;
  }
  ...
  function moment() {
    ...
    const verified=!!state.verified.keyMoment;
    const opts=[['taken','Găsește obiectul, dar cineva îl ia chiar înainte să ajungă la el.'],['morning','Se trezește dimineața.'],['shoes','Își pune pantofii.'],['danger','Descoperă că obiectul se află într-un loc periculos.'],['choice','Trebuie să aleagă între a continua singur sau a cere ajutor.']];
    return `<section class="content-card"><p class="step-kicker">7 · Alege momentul important</p><h2>Când devine aventura intensă?</h2><p class="intro">Personajul tău trebuie să găsească un obiect înainte de apus. Bifează momentele care pot fi importante, apoi verifică.</p><div class="choice-list">${opts.map(([v,l])=>{const picked=selected('keyMoment',v),right=correct.includes(v),cls=verified&&(picked||right)?(right?'is-correct':'is-wrong'):'';return `<label class="choice ${cls} ${verified&&right&&!picked?'correct-answer':''}"><input type="checkbox" data-multi="keyMoment" value="${v}" ${picked?'checked':''}><span>${l}</span>${verified&&right?'<span class="answer-mark">✓</span>':verified&&picked?'<span class="answer-mark">✕</span>':''}</label>`;}).join('')}</div><button class="btn primary" data-action="check-key">Verifică momentele</button>${verified?`<div class="answer-explanation ${ok?'good':'bad'}">${ok?'✅ Corect: variantele 1, 4 și 5.':'❌ Sunt corecte variantele 1, 4 și 5. Roșu = nu schimbă aventura.'}</div>`:''}</section>`;
  }
  ...
  function bind() {
    ...
    app.querySelectorAll('[data-choice]').forEach(el => el.addEventListener('change', event => { state.answers[event.target.dataset.choice] = event.target.value; if (event.target.dataset.choice === 'piesa') state.verified.piesa = false; Object.keys(state.verified).forEach(key => { if (key === 'intelegere') state.verified[key] = false; }); save(); render(); }));
    app.querySelectorAll('[data-multi]').forEach(el => el.addEventListener('change', event => {
      const group = event.target.dataset.multi; const values = [...app.querySelectorAll(`[data-multi="${CSS.escape(group)}"]:checked`)].map(item => item.value); state.answers[group] = values; if (group === 'missing') state.verified.missing = false; if (group === 'keyMoment') state.verified.keyMoment = false;
      if (group === 'solution') state.solutionChoices = values;
      save(); render();
    }));
     app.querySelectorAll('[data-sort]').forEach(el => el.addEventListener('change', event => { state.sort[event.target.dataset.sort] = event.target.value; state.sortChecked = false; save(); render(); }));
    app.querySelectorAll('[data-action]').forEach(el => el.addEventListener('click', event => {
      ...
      if (action === 'check-quiz6') { state.verified.intelegere = true; save(); render(); }
      if (action === 'check-piesa') { state.verified.piesa = true; save(); render(); }
      if (action === 'check-sort') { state.sortChecked = true; save(); render(); }
      if (action === 'check-missing') { state.verified.missing = true; save(); render(); }
      if (action === 'check-key') { state.verified.keyMoment = true; save(); render(); }
    }));
